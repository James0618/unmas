...
class UNMASAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(UNMASAgent, self).__init__()
        self.args = args
        ...
        self.enemies_hidden = nn.Linear(self.enemy_shape, args.rnn_hidden_dim)
        self.q_value_enemy = nn.Sequential(
            nn.Linear(2 * args.rnn_hidden_dim, args.rnn_hidden_dim // 2),
            nn.ReLU(),
    ...
    def forward(self, inputs, hidden_state):
        ...
        hidden_enemies = self.enemies_hidden(enemies_features)
        temp = torch.zeros(hidden_enemies.shape).to(hidden_enemies.device)
        for i in range(self.args.n_agents):
            temp[:, i, :] = inter_h

        hidden = torch.cat((temp, hidden_enemies), dim=-1)
        q_enemies = self.q_value_enemy(hidden).view(inputs.shape[0], -1)
